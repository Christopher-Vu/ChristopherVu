---
title: "Trace Analysis and Learned Kernel Regression Detection on LLM Inference Workloads"
date: "2026-04-01"
...
**Results** This setup matched high confidence (>0.78) scores on almost all traces (even in MoE builds), and revealed >70k kernels present in some build patterns but absent in others. AutoViz<sup>[1]</sup> became the standard trace ingestion method for our 8 person team, processing over 12M kernels.
...
**Results** Unfortunately, there's no ground truth for truly 'anomalous' runs, but we can get close with synthetic patterns. Using real trace patterns, 500 of each of the following anomalous pattern types were generated and compiled into an evaluation, yielding promising results from the autoencoder: magnitude spike (pick one kernel at random and quintuple<sup>[2]</sup> its length, recall 100%), idle stall (add 5 microseconds to a kernel's idle time, recall 98.4%), swap (swap two kernels 3 positions apart, recall 100%), broad drift (multiply all kernel runtimes by 1.1x, recall 69.4%).

<hr />

<sup>[1]</sup> *Aside: 'AutoAnal' was my first name for this functionality, short for auto-analyze. I switched the name to AutoViz for obvious reasons.*

<sup>[2]</sup> *As unrealistic as this sounds, a quintuple wall time kernel was not unusual. This was more of a sanity test (ensure that we don't lose the 2σ test functionality), whereas the real stress test for this model was the broad drift eval.*
